...
(function(){
	...
	login.queryDictionaryInfo = function(isAsync) {
		//Ajax请求业务字典信息
		Ext.Ajax.request({
			url:'../bse/dataDictionaryAction!searchAllDataDictionary.action',
			method: 'POST',
			async: isAsync || false,
	...
	login.isDictionaryHasChanged = function() {
		//判断数据字典内容是否有更新
		Ext.Ajax.request({
			url: '../bse/dataDictionaryAction!isDictionaryHasChanged.action',
			method: 'POST',
			params: {
...
Ext.define('UserContext', {
	...
	//判断当前用户是否有
	isButtonHide : function(functionCode){
		if(login.currentUser && login.currentUser.functionCodes){
			var functionCodes = login.currentUser.functionCodes;
			if(Ext.isArray(functionCodes)){
				if(Ext.Array.contains(functionCodes, functionCode)){
					return false;
				}
			}
		}
