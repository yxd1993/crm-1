 ...
 * 自定义创建表SQL Model
 */
Ext.define('QueryCreateSqlModel', {
...
//自定义查询SQL Store
Ext.define('QueryCreateSqlStore', {
	extend : 'Ext.data.Store',
	model : 'QueryCreateSqlModel',
	pageSize : 15,// 分页条数
	// 数据库取数据方法，暂时屏蔽
	proxy : {
		type : 'ajax',
	...
	*/
	...
	var form=Ext.create('Ext.form.Panel',{
		...
		items:[{
			...
			handler:function(){
				isFirstQuery=1;
				queryCreateSqlStore.loadPage(1);	//查询后默认返回第一页
			}
		}
		]
	});
	
	var queryCreateSqlStore=Ext.create('QueryCreateSqlStore',{
		listeners: {
			beforeload : function(store, operation, eOpts) {
				if (form!= null) {
					...
					var params = {
							...
							'queryCreateSql.createType':'1',
						};
					Ext.apply(store.proxy.extraParams,params );
				}
			},
			load: function(store, operation, eOpts) {
				   if(isFirstQuery==1 && queryCreateSqlStore.getCount()==0){		
	...
	queryCreateSqlStore.loadPage(1);	
...
//	});
	...
	var grid=Ext.create('Ext.grid.Panel', {
		store: queryCreateSqlStore,//创建queryCreateSqlStore
		border:false,
		columnLines:true,
        ...
        tbar:[{
			...
			handler:function(){
				...
				form.reset();
				var queryCreateSqlModel = new QueryCreateSqlModel();
			...
			handler:function(){
				...
				formUrl = '../customer/toVoidqueryCreateSql.action';
				
				if(records.length > 0){
					var ids = new Array();//定义要删除的id对应后台String[] ids
					for (var j = 0; j < records.length; j++) {
						var querySqlModel = new QueryCreateSqlModel();
	...
	*/
	...
	function checkformData(){
		...
		if(id == null || id == ""){
			...
			if(createSql.indexOf('CREATE TABLE') != 0){
				showInfoMsg("创建表SQL语法错误！开头必须包含[CREATE TABLE]");
				return false;
			}
			if(createSql.indexOf(tablenum) <= 0){
				showInfoMsg("表名和创建表SQL不一致！");
				return false;
			}
		}else{
			var alterSql= form.findField("alter").getValue().toUpperCase();
			var tablenum = form.findField("tablenum").getValue().toUpperCase();
			var createSql= form.findField("createSql").getValue().toUpperCase();
